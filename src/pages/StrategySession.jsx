...
export default function StrategySession() {
  ...
  if (sent) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#0B1B27] px-5 text-white">
        <div className="max-w-xl text-center">
          <CheckCircle2 className="mx-auto h-12 w-12 text-[#C5A059]" />
          <h1 className="mt-7 font-heading text-5xl">{t('strategy.successTitle')}</h1>
          <p className="mt-5 leading-relaxed text-white/60">{t('strategy.successBody')}</p>
          <Link to="/" className="mt-8 inline-flex border-b border-[#C5A059] pb-2 text-[#C5A059]">{t('strategy.successBack')}</Link>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-[#0B1B27]">
      <div className="grid min-h-screen lg:grid-cols-[34%_66%]">
        <aside className="hidden bg-[#14222E] p-12 text-white lg:flex lg:flex-col lg:justify-between">
          <Link to="/" className="flex items-center gap-3 font-heading text-xl">
            <Compass className="text-[#C5A059]" /> Externa
          </Link>
        ...
        <section className="flex items-center px-5 py-10 sm:px-12 lg:px-[8vw]">
          <form onSubmit={submit} className="mx-auto w-full max-w-2xl">
            <Link to="/" className="mb-10 inline-flex items-center gap-2 text-sm text-white/60 lg:hidden">
            ...
            <div className="mt-9 flex items-center justify-between">
              {step > 1 ? (
                <button type="button" onClick={() => setStep(step - 1)} className="flex items-center gap-2 text-sm text-white/55">
                  <ArrowLeft className="h-4 w-4" /> {t('strategy.back')}
                </button>
              ) : <span />}
              {step === 3 && (
                <button disabled={!ready || saving} className="flex items-center gap-3 bg-[#C5A059] px-6 py-4 font-semibold text-[#0B1B27] disabled:opacity-40">
