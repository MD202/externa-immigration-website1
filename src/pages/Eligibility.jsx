    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#F8FAFC] px-5 text-[#1E2A4A]">
        <div className="max-w-xl text-center">
          ...
          <p className="mt-5 leading-relaxed text-[#1E2A4A]/60">{t('eligibility.successBody')}</p>
          <Link to="/" className="mt-8 inline-flex border-b border-[#B8860B] pb-2 text-[#B8860B]">{t('eligibility.successBack')}</Link>
        </div>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-[#F8FAFC]">
      ...
      <section className="px-5 pt-32 pb-20 lg:px-[8vw] lg:pt-40">
        <div className="mx-auto max-w-3xl">
          ...
          {!isResult ? (
            <div className="mt-12">
              <div className="mb-6 h-1 w-full bg-[#1E2A4A]/10">
                <div className="h-1 bg-[#B8860B] transition-all" style={{ width: `${(step / total) * 100}%` }} />
              </div>
              <p className="text-xs font-semibold uppercase tracking-[.18em] text-[#1E2A4A]/50">
                {t('eligibility.stepOf').replace('{n}', String(step + 1)).replace('{total}', String(total))}
              </p>
              <h2 className="mt-3 font-heading text-3xl text-[#1E2A4A]">{t(`eligibility.${QUESTIONS[step].key}`)}</h2>
              <div className="mt-8 grid gap-3">
                {QUESTIONS[step].options.map((opt) => (
                  <button key={opt} type="button" onClick={() => choose(QUESTIONS[step].key, opt)} className="flex items-center justify-between gap-4 border border-[#1E2A4A]/10 bg-white px-6 py-5 text-left transition hover:border-[#B8860B] hover:shadow-md">
                    <span className="text-base text-[#1E2A4A]/80">{t(`eligibility.${opt}`)}</span>
                    <ArrowRight className="h-5 w-5 shrink-0 text-[#B8860B]" />
                  </button>
                ))}
              </div>
              {step > 0 && (
                <button type="button" onClick={() => setStep(step - 1)} className="mt-8 flex items-center gap-2 text-sm text-[#1E2A4A]/55">
            ...
            <div className="mt-12 grid gap-8 lg:grid-cols-2">
              <div>
                ...
                <p className="mt-4 text-base leading-relaxed text-[#1E2A4A]/65">{svc ? t(svc.descKey) : ''}</p>
                <p className="mt-6 border-l-2 border-[#B8860B] bg-white px-5 py-4 text-sm italic leading-relaxed text-[#1E2A4A]/60">{t('eligibility.resultNote')}</p>
                <Link to="/strategy-session" className="mt-6 inline-flex items-center gap-2 border-b-2 border-[#B8860B] pb-1 text-sm font-semibold text-[#B8860B]">
              ...
              <form onSubmit={submit} className="bg-white p-8 shadow-sm">
                ...
                <button type="submit" disabled={saving} className="mt-7 flex w-full items-center justify-center gap-2 bg-[#B8860B] px-6 py-4 font-semibold text-[#FFFFFF] transition hover:bg-[#8B6508] disabled:opacity-50">
