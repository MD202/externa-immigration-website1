    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#F4F7F9] px-5 text-[#0F2433]">
        <div className="max-w-xl text-center">
          ...
          <p className="mt-5 leading-relaxed text-[#0F2433]/60">{t('eligibility.successBody')}</p>
          <Link to="/" className="mt-8 inline-flex border-b border-[#C8102E] pb-2 text-[#C8102E]">{t('eligibility.successBack')}</Link>
        </div>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-[#F4F7F9]">
      ...
      <section className="px-5 pt-32 pb-20 lg:px-[8vw] lg:pt-40">
        <div className="mx-auto max-w-3xl">
          ...
          <p className="mt-6 max-w-2xl text-lg leading-relaxed text-[#0F2433]/65">{t('eligibility.subtitle')}</p>

          {!isResult ? (
            <div className="mt-12">
              <div className="mb-6 h-1 w-full bg-[#0F2433]/10">
                <div className="h-1 bg-[#C8102E] transition-all" style={{ width: `${(step / total) * 100}%` }} />
              </div>
              <p className="text-xs font-semibold uppercase tracking-[.18em] text-[#0F2433]/50">
                {t('eligibility.stepOf').replace('{n}', String(step + 1)).replace('{total}', String(total))}
              </p>
              <h2 className="mt-3 font-heading text-3xl text-[#0F2433]">{t(`eligibility.${QUESTIONS[step].key}`)}</h2>
              <div className="mt-8 grid gap-3">
                {QUESTIONS[step].options.map((opt) => (
                  <button key={opt} type="button" onClick={() => choose(QUESTIONS[step].key, opt)} className="flex items-center justify-between gap-4 border border-[#0F2433]/10 bg-white px-6 py-5 text-left transition hover:border-[#C8102E] hover:shadow-md">
                    <span className="text-base text-[#0F2433]/80">{t(`eligibility.${opt}`)}</span>
                    <ArrowRight className="h-5 w-5 shrink-0 text-[#C8102E]" />
                  </button>
                ))}
              </div>
              {step > 0 && (
                <button type="button" onClick={() => setStep(step - 1)} className="mt-8 flex items-center gap-2 text-sm text-[#0F2433]/55">
            ...
            <div className="mt-12 grid gap-8 lg:grid-cols-2">
              <div>
                ...
                <h2 className="mt-3 font-heading text-3xl text-[#0F2433]">{svc ? t(svc.titleKey) : ''}</h2>
                <p className="mt-4 text-base leading-relaxed text-[#0F2433]/65">{svc ? t(svc.descKey) : ''}</p>
                <p className="mt-6 border-l-2 border-[#C5A059] bg-white px-5 py-4 text-sm italic leading-relaxed text-[#0F2433]/60">{t('eligibility.resultNote')}</p>
                <Link to="/strategy-session" className="mt-6 inline-flex items-center gap-2 border-b-2 border-[#C8102E] pb-1 text-sm font-semibold text-[#C8102E]">
                  {t('eligibility.bookConsult')} <ArrowRight className="h-4 w-4" />
                </Link>
              </div>
              <form onSubmit={submit} className="bg-white p-8 shadow-sm">
                <h3 className="font-heading text-2xl text-[#0F2433]">{t('eligibility.formTitle')}</h3>
                <p className="mt-2 text-sm leading-relaxed text-[#0F2433]/55">{t('eligibility.formSubtitle')}</p>
                ...
                <button type="submit" disabled={saving} className="mt-7 flex w-full items-center justify-center gap-2 bg-[#C8102E] px-6 py-4 font-semibold text-white transition hover:bg-[#A00D24] disabled:opacity-50">
