...
export default function Fees() {
  ...
  return (
    <main className="overflow-hidden bg-[#F4EEE2]">
      <NorthStarCursor />
      <Header />
      <section className="px-5 pt-32 pb-20 lg:px-[8vw] lg:pt-40">
        <div className="mx-auto max-w-[1440px]">
          ...
          <Link to="/" className="mb-10 flex items-center gap-3 lg:hidden">
            <Compass className="h-8 w-8 text-[#C9A227]" />
            <span className="font-heading text-lg leading-tight text-[#123B2C]">Externa<span className="mt-0.5 block font-body text-[9px] uppercase tracking-[.22em] text-[#123B2C]/55">Immigration Solutions Inc</span></span>
          </Link>
          <p className="eyebrow">{t('fees.eyebrow')}</p>
          <h1 className="section-title">{t('fees.title')}</h1>
          <p className="mt-6 max-w-2xl text-lg leading-relaxed text-[#123B2C]/65">{t('fees.intro')}</p>
          ...
          <div className="mt-14 flex flex-col items-start justify-between gap-6 border-t border-[#123B2C]/10 pt-10 md:flex-row md:items-center">
            <div>
          ...
          <div className="mt-16">
            <h2 className="font-heading text-3xl text-[#123B2C]">Full representation fees</h2>
            <div className="mt-8 grid gap-5 md:grid-cols-2">
              {items.map((item) => (
                <div key={item.title} className="flex flex-col justify-between border border-[#123B2C]/8 bg-white p-7 shadow-sm transition hover:shadow-md">
                  <div className="flex items-start justify-between gap-4">
                    <h3 className="font-heading text-2xl text-[#123B2C]">{item.title}</h3>
                    <span className="whitespace-nowrap font-heading text-xl text-[#C9A227]">{item.fee}</span>
                  </div>
                  <p className="mt-4 text-sm leading-relaxed text-[#123B2C]/55">{item.desc}</p>
                </div>
              ))}
            </div>
          </div>
          <p className="mt-8 max-w-2xl text-sm leading-relaxed text-[#123B2C]/45">{t('fees.note')}</p>
          <p className="mt-3 max-w-2xl text-sm font-semibold leading-relaxed text-[#C9A227]">{t('fees.exclusion')}</p>
          <div className="mt-6 max-w-2xl border-l-2 border-[#C9A227] bg-[#F4EEE2] px-5 py-4">
            <p className="text-sm leading-relaxed text-[#123B2C]/70">{t('fees.stagedNote')}</p>
          </div>
          <Link to="/strategy-session" className="mt-8 inline-flex items-center gap-3 bg-[#C9A227] px-6 py-4 font-semibold text-[#3D2F06] transition hover:bg-[#A8871A]">{t('fees.cta')} <ArrowUpRight /></Link>
        </div>
      </section>
      <Footer />
      <WhatsAppWidget />
