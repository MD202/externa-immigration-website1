          ...
          <div key={s.n} className="flex flex-1 items-start last:flex-none">
            <div className="flex flex-col items-center">
              <div className={`flex h-9 w-9 items-center justify-center rounded-full border-2 text-sm font-semibold transition ${
                step > s.n ? 'border-[#A85638] bg-[#A85638] text-white' :
                step === s.n ? 'border-[#A85638] bg-white text-[#A85638]' :
            ...
            {i < steps.length - 1 && <div className={`mx-2 mt-4 h-px flex-1 ${step > s.n ? 'bg-[#A85638]' : 'bg-[#0E3B3B]/15'}`} />}
          </div>
        ))}
      </div>
    </div>
