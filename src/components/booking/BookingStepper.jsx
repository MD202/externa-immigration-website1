          ...
          <div key={s.n} className="flex flex-1 items-start last:flex-none">
            <div className="flex flex-col items-center">
              <div className={`flex h-9 w-9 items-center justify-center rounded-full border-2 text-sm font-semibold transition ${
                step > s.n ? 'border-[#C9A227] bg-[#C9A227] text-[#3D2F06]' :
                step === s.n ? 'border-[#C9A227] bg-white text-[#C9A227]' :
                'border-[#123B2C]/20 bg-white text-[#123B2C]/40'
              }`}>
                {step > s.n ? <Check className="h-4 w-4" /> : s.n}
              </div>
              <span className={`mt-2 text-center text-xs ${step >= s.n ? 'text-[#123B2C]' : 'text-[#123B2C]/40'}`}>{s.label}</span>
            </div>
            {i < steps.length - 1 && <div className={`mx-2 mt-4 h-px flex-1 ${step > s.n ? 'bg-[#C9A227]' : 'bg-[#123B2C]/15'}`} />}
          </div>
        ))}
      </div>
    </div>
