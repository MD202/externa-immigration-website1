...
export default function TimeStep({ data, setData }) {
  ...
  return (
    <div>
      <h2 className="font-heading text-2xl text-[#0E3B3B]">{t('bookingFlow.selectTime')}</h2>
      <p className="mt-2 text-sm text-[#0E3B3B]/50">{serviceName} — {serviceDuration}</p>
      ...
      <div className="mt-6 grid gap-5 sm:grid-cols-2">
        ...
        <div className="intake-label">{t('bookingFlow.selectTimeLabel')}
          {loading ? (
            <p className="mt-3 flex items-center gap-2 text-sm text-[#0E3B3B]/50"><Loader2 className="h-4 w-4 animate-spin" /> {t('bookingFlow.loadingSlots')}</p>
          ) : data.preferred_date ? (
            slots.length > 0 ? (
              <div className="mt-3 flex flex-wrap gap-2">
                {slots.map((time) => (
                  <button key={time} type="button" onClick={() => setData((d) => ({ ...d, preferred_time: time }))} className={`border px-3 py-2 text-sm transition ${data.preferred_time === time ? 'border-[#A85638] bg-[#A85638] text-white' : 'border-[#0E3B3B]/20 text-[#0E3B3B]/70 hover:border-[#A85638]'}`}>{time}</button>
                ))}
              </div>
            ) : (
              <p className="mt-3 text-sm text-[#0E3B3B]/50">{t('bookingFlow.noSlots')}</p>
            )
          ) : (
            <p className="mt-3 text-sm text-[#0E3B3B]/40">{t('bookingFlow.pickDate')}</p>
          )}
        </div>
      </div>

      <p className="mt-4 text-xs text-[#0E3B3B]/40">{t('bookingFlow.timeZone')}</p>
    </div>
  );
}