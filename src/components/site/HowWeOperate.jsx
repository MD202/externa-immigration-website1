      ...
      <div className="mx-auto max-w-[1440px]">
        ...
        <div className="mt-16 hidden lg:block">
          <div className="relative">
            ...
            <div className="absolute top-5 left-5 h-px bg-[#047857] transition-all duration-500" style={{ width: `calc(${active / (steps.length - 1)} * (100% - 40px))` }} />
              ...
              {steps.map((step, i) => (
                <button key={step.n} onClick={() => setActive(i)} className="flex flex-col items-center" aria-label={step.label}>
                  <span className={`flex h-10 w-10 items-center justify-center rounded-full border-2 font-mono text-sm font-semibold transition ${
                    active === i
                      ? 'border-[#047857] bg-[#047857] text-[#FFFFFF]'
                      : i < active
                        ? 'border-[#047857] bg-[#047857]/15 text-[#047857]'
                        : 'border-white/25 bg-[#1E293B] text-white/50 hover:border-white/50 hover:text-white'
                  }`}>{step.n}</span>
                </button>
              ))}
            </div>
        ...
        <div className="mt-12 grid gap-3 lg:hidden">
          {steps.map((step, i) => (
            <button key={step.n} onClick={() => setActive(i)} className={`flex items-center gap-4 border p-4 text-left transition ${
              active === i ? 'border-[#047857] bg-[#047857]/10' : 'border-white/10'
            }`}>
              <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-2 font-mono text-sm font-semibold ${
                active === i ? 'border-[#047857] bg-[#047857] text-[#FFFFFF]' :
                i < active ? 'border-[#047857] bg-[#047857]/15 text-[#047857]' :
        ...
        <Link to="/strategy-session" className="mt-12 inline-flex items-center gap-3 bg-[#047857] px-6 py-4 font-semibold text-[#FFFFFF] transition hover:bg-[#065F46]">
