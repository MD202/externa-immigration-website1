...
export default function BusinessFeature() {
  const { t } = useLanguage();
  return (
    <section className="relative min-h-[680px] overflow-hidden bg-[#0F2433] text-white">
      ...
      <div className="absolute inset-0 bg-gradient-to-r from-[#0F2433] via-[#0F2433]/80 to-transparent" />
      <div className="relative mx-auto flex min-h-[680px] max-w-[1440px] items-center px-5 lg:px-[8vw]">
        <div className="max-w-2xl">
          ...
          <Link to="/strategy-session" className="mt-9 inline-flex items-center gap-3 bg-[#C8102E] px-6 py-4 font-semibold text-white transition hover:bg-[#A00D24]">{t('business.cta')} <ArrowUpRight /></Link>
        </div>
      </div>
    </section>
  );
