...
export default function BusinessFeature() {
  const { t } = useLanguage();
  return (
    <section className="relative min-h-[680px] overflow-hidden bg-[#1E293B] text-white">
      ...
      <div className="relative mx-auto flex min-h-[680px] max-w-[1440px] items-center px-5 lg:px-[8vw]">
        <Reveal className="max-w-2xl">
          ...
          <h2 className="section-title text-white">{t('business.title1')}<br /><em className="text-[#047857]">{t('business.title2')}</em></h2>
          <p className="mt-7 max-w-xl text-lg leading-relaxed text-white/65">{t('business.body')}</p>
          <Link to="/strategy-session" className="mt-9 inline-flex items-center gap-3 bg-[#047857] px-6 py-4 font-semibold text-[#FFFFFF] transition hover:bg-[#065F46]">{t('business.cta')} <ArrowUpRight /></Link>
        </Reveal>
      </div>
    </section>
  );
