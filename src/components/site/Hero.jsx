    ...
    <section id="top" className="relative min-h-screen overflow-hidden bg-[#0F2433] text-white">
      ...
      <div className="relative mx-auto grid min-h-screen max-w-[1440px] items-center gap-12 px-5 pb-16 pt-32 lg:grid-cols-12 lg:px-[8vw]">
        <div className="lg:col-span-7">
          <div className="mb-7 flex items-center gap-3 text-xs uppercase tracking-[.24em] text-[#C5A059]"><BadgeCheck className="h-5 w-5" /> {t('hero.eyebrow')}</div>
          ...
          <div className="mt-9 flex flex-wrap gap-4">
            <Link to="/strategy-session" className="group flex items-center gap-3 bg-[#C8102E] px-6 py-4 font-semibold text-white transition hover:bg-[#A00D24]">{t('hero.cta1')} <ArrowUpRight className="transition-transform group-hover:-translate-y-1 group-hover:translate-x-1" /></Link>
            <a href="#services" className="border border-white/30 px-6 py-4 font-semibold transition hover:bg-white/10">{t('hero.cta2')}</a>
          </div>
        </div>
        <aside className="border border-white/15 bg-white/10 p-6 backdrop-blur-xl lg:col-span-5 lg:p-8">
