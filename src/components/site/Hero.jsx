      ...
      <div className="relative mx-auto grid min-h-screen max-w-[1440px] items-center gap-12 px-5 pb-16 pt-32 lg:grid-cols-12 lg:px-[8vw]">
        ...
        <aside className="hero-in border border-white/15 bg-white/10 p-6 backdrop-blur-xl lg:col-span-5 lg:p-8" style={{ animationDelay: '0.75s' }} aria-label="Quick assessment">
          ...
          <div className="mt-6 grid gap-1">
            {intents.map((item) => (
              <a key={item.key} href={item.href} className="group flex items-center justify-between border-b border-white/15 py-3.5 text-left text-sm text-white/80 transition hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-[#047857]">
                {t(item.key)}
                <ChevronRight className="h-4 w-4 text-[#047857] transition group-hover:translate-x-1" aria-hidden="true" />
              </a>
            ))}
          </div>
