        ...
        <div className="hidden items-center gap-8 lg:flex">
          <nav className="flex items-center gap-6" aria-label="Main navigation">
            <div className="relative">
              <button onClick={() => setOpenMenu(openMenu === 'services' ? null : 'services')} className="flex items-center gap-1 text-sm text-[#0E3B3B]/70 transition hover:text-[#6B1E1E]" aria-expanded={openMenu === 'services'}>
              ...
              {openMenu === 'services' && (
                <div className="absolute top-full left-0 pt-2 w-72">
                  <div className="grid gap-0 border border-[#0E3B3B]/10 bg-white py-2 shadow-xl">
                    {services.map((s) => (
                      <Link key={s.id} to={`/services/${s.id}`} onClick={() => setOpenMenu(null)} className="px-4 py-3 text-sm text-[#0E3B3B]/70 transition hover:text-[#6B1E1E]">{s.label}</Link>
                    ))}
                  </div>
                </div>
              )}
            ...
            <div className="relative">
              <button onClick={() => setOpenMenu(openMenu === 'about' ? null : 'about')} className="flex items-center gap-1 text-sm text-[#0E3B3B]/70 transition hover:text-[#6B1E1E]" aria-expanded={openMenu === 'about'}>
              ...
              {openMenu === 'about' && (
                <div className="absolute top-full left-0 pt-2 w-56">
                  <div className="grid gap-0 border border-[#0E3B3B]/10 bg-white py-2 shadow-xl">
                    {aboutLinks.map((link) => (
                      <Link key={link.href} to={link.href} onClick={() => setOpenMenu(null)} className="px-4 py-3 text-sm text-[#0E3B3B]/70 transition hover:text-[#6B1E1E]">{link.label}</Link>
                    ))}
                  </div>
                </div>
              )}
            </div>
            <Link to="/eligibility" className="text-sm font-semibold text-[#6B1E1E] transition hover:text-[#561818]">{t('eligibility.nav')}</Link>
          </nav>
          <span className="h-7 w-px bg-[#0E3B3B]/15" aria-hidden="true" />
          <Link to="/strategy-session" className="bg-[#6B1E1E] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#561818]">{t('nav.book')}</Link>
        </div>
        <button onClick={() => setMobileOpen(!mobileOpen)} className="text-[#0E3B3B] lg:hidden focus:outline-none focus-visible:ring-2 focus-visible:ring-[#6B1E1E] focus-visible:ring-offset-2" aria-label="Toggle navigation" aria-expanded={mobileOpen} aria-controls="mobile-nav">{mobileOpen ? <X /> : <Menu />}</button>
      </div>
      {mobileOpen && (
        <nav id="mobile-nav" className="mx-5 mt-3 max-h-[70vh] overflow-y-auto rounded-lg border border-[#0E3B3B]/10 bg-white py-4 shadow-xl lg:hidden" aria-label="Mobile navigation">
          ...
          <Link onClick={() => setMobileOpen(false)} to="/eligibility" className="block px-4 py-3 font-semibold text-[#6B1E1E]">{t('eligibility.nav')}</Link>
          <div className="my-2 border-t border-[#0E3B3B]/10" />
          <Link to="/strategy-session" onClick={() => setMobileOpen(false)} className="mx-3 mb-3 bg-[#6B1E1E] px-4 py-3 text-center font-semibold text-white">{t('nav.book')}</Link>
        </nav>
      )}
    </header>
  );
