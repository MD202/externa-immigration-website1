    ...
    <header ref={headerRef} style={{ top: topOffset }} className={`fixed inset-x-0 top-0 z-[100] bg-white shadow-sm transition-all duration-300 ${compact ? 'py-3' : 'py-4'}`}>
      <div className="mx-auto flex max-w-[1440px] items-center justify-between px-5 lg:px-[8vw]">
        ...
        <div className="hidden items-center gap-8 lg:flex">
          ...
          <Link to="/strategy-session" className="border border-[#B8860B] bg-white px-5 py-2.5 text-sm font-semibold text-[#B8860B] transition hover:bg-[#B8860B] hover:text-white">Book a consultation</Link>
        </div>
      ...
      {mobileOpen && (
        <nav id="mobile-nav" className="mx-5 mt-3 max-h-[70vh] overflow-y-auto border border-[#1E2A4A]/10 bg-white py-4 lg:hidden" aria-label="Mobile navigation">
          ...
          <Link to="/strategy-session" onClick={() => setMobileOpen(false)} className="mx-4 mt-3 border border-[#B8860B] bg-white px-4 py-3 text-center text-sm font-semibold text-[#B8860B]">Book a consultation</Link>
        </nav>
      )}
    </header>
  );
