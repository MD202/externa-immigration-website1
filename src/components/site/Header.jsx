        ...
        <div className="hidden items-center gap-8 lg:flex">
          <nav className="flex items-center gap-6" aria-label="Main navigation">
            <div className="relative">
              <button onClick={() => setOpenMenu(openMenu === 'services' ? null : 'services')} className="flex items-center gap-1 text-sm text-[#1E2A4A]/70 transition hover:text-[#C9A227]" aria-expanded={openMenu === 'services'}>
              ...
              {openMenu === 'services' && (
                <div className="absolute top-full left-0 pt-2 w-72">
                  <div className="grid gap-0 border border-[#1E2A4A]/10 bg-white py-2 shadow-xl">
                    {services.map((s) => (
                      <Link key={s.id} to={`/services/${s.id}`} onClick={() => setOpenMenu(null)} className="px-4 py-3 text-sm text-[#1E2A4A]/70 transition hover:text-[#C9A227]">{s.label}</Link>
                    ))}
                  </div>
                </div>
              )}
            ...
            <div className="relative">
              <button onClick={() => setOpenMenu(openMenu === 'about' ? null : 'about')} className="flex items-center gap-1 text-sm text-[#1E2A4A]/70 transition hover:text-[#C9A227]" aria-expanded={openMenu === 'about'}>
              ...
              {openMenu === 'about' && (
                <div className="absolute top-full left-0 pt-2 w-56">
                  <div className="grid gap-0 border border-[#1E2A4A]/10 bg-white py-2 shadow-xl">
                    {aboutLinks.map((link) => (
                      <Link key={link.href} to={link.href} onClick={() => setOpenMenu(null)} className="px-4 py-3 text-sm text-[#1E2A4A]/70 transition hover:text-[#C9A227]">{link.label}</Link>
                    ))}
                  </div>
                </div>
              )}
          ...
          <Link to="/strategy-session" className="bg-[#C9A227] px-5 py-3 text-sm font-semibold text-[#FFFFFF] transition hover:bg-[#A8871A]">{t('nav.book')}</Link>
        </div>
        <button onClick={() => setMobileOpen(!mobileOpen)} className="text-[#1E2A4A] lg:hidden focus:outline-none focus-visible:ring-2 focus-visible:ring-[#C9A227] focus-visible:ring-offset-2" aria-label="Toggle navigation" aria-expanded={mobileOpen} aria-controls="mobile-nav">{mobileOpen ? <X /> : <Menu />}</button>
      </div>
      {mobileOpen && (
