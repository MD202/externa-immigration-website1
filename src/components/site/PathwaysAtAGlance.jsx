...
export default function PathwaysAtAGlance() {
  ...
  return (
    <section id="pathways" className="bg-[#1E293B] px-5 py-28 text-white lg:px-[8vw] lg:py-40">
      <div className="mx-auto max-w-[1440px]">
        ...
        <div className="mt-16 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {pathways.map(({ icon: Icon, title, body }, i) => (
            <Reveal key={title} delay={i * 70} className="h-full">
              <article className="group h-full border border-white/10 p-6 transition duration-300 hover:-translate-y-1 hover:border-[#047857]/50 hover:bg-white/[.04]">
                <div className="flex h-11 w-11 items-center justify-center bg-[#047857]/15 text-[#047857] transition group-hover:bg-[#047857] group-hover:text-white">
        ...
        <Reveal>
          <Link to="/eligibility" className="mt-12 inline-flex items-center gap-3 bg-[#047857] px-6 py-4 font-semibold text-white transition hover:bg-[#065F46]">{t('pathways.cta')} <ArrowUpRight className="h-4 w-4" /></Link>
        </Reveal>
      </div>
    </section>
  );
