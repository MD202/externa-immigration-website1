    ...
    <section aria-label="Assessment and process timeline" className="bg-white px-5 py-28 lg:px-[8vw] lg:py-40">
      <div className="mx-auto max-w-[1440px]">
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          {pills.map((pill) => (
            <a key={pill.key} href={pill.href} className="border border-[#13203F]/15 px-5 py-3 text-sm text-[#13203F]/75 transition hover:border-[#B8860B] hover:text-[#B8860B]">
              {t(pill.key)}
            </a>
          ))}
        </div>
        <div className="mt-20 grid gap-12 lg:grid-cols-2 lg:items-center">
          <div>
            ...
            <Link to="/strategy-session" className="mt-8 inline-flex items-center gap-3 bg-[#B8860B] px-6 py-4 font-semibold text-[#FFFFFF] transition hover:bg-[#8B6508]">
          ...
          <ol className="relative grid gap-0 border-l-2 border-[#13203F]/10 pl-0">
            {steps.map((step, i) => (
              <li key={step.n} className="flex items-center gap-5 border-b border-[#13203F]/10 py-6 last:border-0">
                <span className={`flex h-12 w-12 shrink-0 items-center justify-center font-mono text-sm ${step.active ? 'bg-[#B8860B] text-[#FFFFFF]' : 'bg-[#13203F]/8 text-[#13203F]/60'}`}>{step.n}</span>
