  ...
  return (
    <>
      <div id="where" className="hero-in border border-white/14 bg-[#1E2A4A] p-6 lg:p-8" style={{ animationDelay: '0.6s' }}>
        ...
        <div className="mt-5">
          {rows.map((row) => (
            <button key={row.tag} onClick={() => go(row)} className="group relative flex w-full items-center gap-4 border-b border-white/14 py-4 text-left transition-colors last:border-b-0 hover:bg-white/[.04]">
