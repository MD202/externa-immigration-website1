...
export default function ApproachSection() {
  ...
  return (
    <section id="approach" className="bg-[#F4EEE2] px-5 py-28 lg:px-[8vw] lg:py-40">
      <div className="mx-auto grid max-w-[1440px] gap-14 lg:grid-cols-12 lg:items-center">
        ...
        <div className="lg:col-span-5 lg:col-start-8">
          ...
          <p className="mt-6 text-lg leading-relaxed text-[#123B2C]/65">{t('approach.intro')}</p>
          <div className="mt-9 grid gap-5">
            {principles.map((item) => (
              <div key={item.n} className="flex gap-5 border-b border-[#123B2C]/10 pb-5">
                <span className="font-mono text-sm font-semibold text-[#C9A227]">{item.n}</span>
                <span className="text-[#123B2C]/80">{item.text}</span>
              </div>
            ))}
          </div>
        </div>
