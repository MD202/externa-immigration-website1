...
export default function ServicesGrid() {
  ...
  return (
    <section id="services" className="bg-[#F4F7F9] px-5 py-28 lg:px-[8vw] lg:py-40">
      <div className="mx-auto max-w-[1440px]">
        ...
        <div className="mt-16 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {services.map(({ icon: Icon, title, text, factors }) => (
            <Link to="/strategy-session" key={title} className="group flex flex-col justify-between bg-white p-7 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl">
              <div className="flex justify-between">
                ...
                <ArrowUpRight className="h-5 w-5 text-[#0F2433]/20 transition group-hover:text-[#C8102E]" />
              </div>
              <div className="mt-8">
                <h3 className="font-heading text-2xl text-[#0F2433]">{title}</h3>
                <p className="mt-3 leading-relaxed text-[#0F2433]/60">{text}</p>
