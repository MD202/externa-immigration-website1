...
// Conversion hero. Optional priceLine / rcicLine / ctaLabel let a page customize
// the price caption, the regulatory line, and the button label; defaults keep
// the simpler entrepreneur hero working unchanged.
export default function PathwayHero({ eyebrow, title, subtitle, trustLine, launchDate, priceLabel, priceLine, rcicLine, ctaLabel }) {
  return (
    <section className="relative overflow-hidden bg-[#1E2A4A] px-5 pb-20 pt-32 text-white lg:px-[8vw] lg:pb-28 lg:pt-40">
      ...
      <div className="relative mx-auto max-w-4xl text-center">
        ...
        <div className="mt-9 flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
          <a href="#register" className="btn btn-primary w-full sm:w-auto">{ctaLabel || 'Register now'} <ArrowRight className="h-4 w-4" /></a>
          <span className="text-sm text-white/60">{priceLine || `Just ${priceLabel} +HST · single payment`}</span>
        </div>
        {rcicLine ? (
          <p className="mt-4 text-xs uppercase tracking-[.18em] text-white/40">{rcicLine}</p>
        ) : (
